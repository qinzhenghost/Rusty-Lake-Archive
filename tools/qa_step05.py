...
language_hook = (ROOT/'src/components/useArchiveLanguage.ts').read_text('utf-8')
check('reader:language-persistence', 'useArchiveLanguage' in reader and 'rla-language-v1' in language_hook and 'localStorage.setItem(LANGUAGE_KEY' in language_hook, 'global language mode is not persisted')
...
check('reader:source-label', "'SOURCE'" in reader and "'来源'" in reader and 'sourceIds.join' in reader, 'drawer provenance missing')
